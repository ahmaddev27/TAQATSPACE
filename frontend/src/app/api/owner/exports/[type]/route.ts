...
/**
 * GET /api/owner/exports/{type}?<filters>
 *
 * The backend streams the owner's OWN workspace CSV exports behind a Sanctum
 * bearer token. The browser can't attach our httpOnly cookie's token to a
 * cross-origin `<a download>`, so this same-origin handler reads the cookie
 * server-side, forwards the list filters as a query string, calls the backend
 * with the bearer, and pipes the CSV straight back as a file attachment. The
 * backend scopes the data to the authenticated owner's workspace.
 */
export async function GET(
  ...
  // Carry through the owner list filters (status, search, month) untouched.
  const query = new URL(request.url).search;
  const base = API_BASE.replace(/\/$/, "");

  let upstream: Response;
  try {
    upstream = await fetch(`${base}/workspace/exports/${type}${query}`, {
      headers: {
